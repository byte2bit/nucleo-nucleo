...
include_once dirname(elfinder_connector_path()) . DIRECTORY_SEPARATOR . '/editors/editor.php';
include_once dirname(elfinder_connector_path()) . DIRECTORY_SEPARATOR . '/libs/GdBmp.php';
include_once dirname(elfinder_connector_path()) . DIRECTORY_SEPARATOR . '/elFinderPlugin.php';
include_once dirname(elfinder_connector_path()) . DIRECTORY_SEPARATOR . '/plugins/AutoResize/plugin.php';
include_once dirname(elfinder_connector_path()) . DIRECTORY_SEPARATOR . '/plugins/AutoRotate/plugin.php';
include_once dirname(elfinder_connector_path()) . DIRECTORY_SEPARATOR . '/plugins/Normalizer/plugin.php';
include_once dirname(elfinder_connector_path()) . DIRECTORY_SEPARATOR . '/plugins/Sanitizer/plugin.php';
include_once dirname(elfinder_connector_path()) . DIRECTORY_SEPARATOR . '/plugins/Watermark/plugin.php';
include_once dirname(elfinder_connector_path()) . DIRECTORY_SEPARATOR . '/elFinderSessionInterface.php';
include_once dirname(elfinder_connector_path()) . DIRECTORY_SEPARATOR . '/elFinderSession.php';
...
class elFinderConnectorController implements ContainerInjectionInterface {
  ...
  public function getConnector(Request $request) {
    ...
    $options_defs = array(
      ...
      'userProfile' => $profile,
      'dispInlineRegex' => \Drupal::config('elfinder.settings')->get('filesystem.inlinepreviewregex'),
      'maxArcFilesSize' => \Drupal::config('elfinder.settings')->get('filesystem.maxarchivesize'),
    ...
    if (is_object($pubFiles) && \Drupal::currentUser()->hasPermission('access public files')) {
      ...
      $roots[] = array_merge($options_defs, array(
        'driver' => 'Drupal',
        'path' => \Drupal::service('file_system')->realpath('public://'), // path to root directory (named 'path' in elFinder 2.0)
    ...
    if (is_object($pvtFiles) && \Drupal::currentUser()->hasPermission('access private files')) {
      ...
      $roots[] = array_merge($options_defs, array(
        'driver' => 'Drupal',
        'path' => \Drupal::service('file_system')->realpath('private://'), // path to root directory (named 'path' in elFinder 2.0)
      ...
      foreach ($profile->getConf('volumes') as $volume) {
        ...
        if ($is_subdir) {
          ...
          if (!file_prepare_directory($rootpath, FILE_CREATE_DIRECTORY)) {
            \Drupal::messenger()->addMessage(t('Error. Cannot initialize directory %dir', array('%dir' => $rootpath)), 'error');
          }

          $root['path'] = \Drupal::service('file_system')->realpath($rootpath);
          $root['URL'] = $url;
    ...
    if ($elFinderObj == null) {
      return new JsonResponse(array('error' => array(t('Unable to initialize elFinder object'))));
    }
